...
Симулятор 2D-среды (препятствия, градиенты ресурса/токсина, статический/динамический режим) детерминирован по ENV\_PISTREAM.



Агент: кинематика + столкновения + сенсоры (3–5 “ресничек”-лучей, 1–2 градиентных датчика) + контролируемый шум по NOISE\_PISTREAM.



qcore (S={T,F,MY,MN} + conflict\_flag) реализован как библиотека, с truth tables и unit/regression тестами.
...
Evolution loop: популяция/селекция/мутации, строго детерминированные MUT\_DECISION\_PISTREAM и MUT\_MAGNITUDE\_PISTREAM.
...
В репо есть RUN\_COMMANDS.md, EXPERIMENT\_SPEC.md, DECISIONS/…, и всё логируется в AGENTS\_LOG.md.
...
RUN\_COMMANDS.md (пустой шаблон + правила заполнения)



EXPERIMENT\_SPEC.md (шаблон фиксации методики)
...
Файлы присутствуют, AGENTS\_LOG.md обновлён
...
qcore/ модуль: NOT/AND/OR/CONSENSUS/CONFLICT + conflict\_flag



qcore/\_\_tests\_\_/truth\_tables.test.\* (референс-таблицы)
...
qcore/\_\_tests\_\_/regression.seeded.test.\* (фиксированные кейсы)
...
Поддержка потоков: ENV/INIT/NOISE/MUT\_DECISION/MUT\_MAGNITUDE
...
любой эксперимент пишет seed\_manifest.json
...
TASK-0400: World model + генерация (ENV\_PISTREAM)
...
env/generate.ts: генерация по ENV\_PISTREAM
...
TASK-0501: Сенсоры (ciliary rays + gradient sensors) + NOISE\_PISTREAM
...
agent/controller.ts: выдаёт S и conflict\_flag



правило: если conflict\_flag=1 → обязательный safe-override (MVP: снижение скорости/поворот/скан)
...
experiments/damage-model.ts: формальные повреждения (bit flips, удаление сегмента длины L, etc.) — фиксируются в EXPERIMENT\_SPEC.md
...
UNVERIFIED (нужно зафиксировать в EXPERIMENT\_SPEC.md): точная формула utility.
...
TASK-0701: Mutations (MUT\_DECISION/MUT\_MAGNITUDE PiStreams)
...
TASK-0800: EXPERIMENT\_SPEC.md (фиксация методики до запуска)
...
scripts/run\_paramecium.ts (или аналог)
...
RESULTS/run\_<timestamp>/seed\_manifest.json



RESULTS/run\_<timestamp>/raw.csv
...
pnpm exp:paramecium --seed <X> --n 30 → генерирует RESULTS/\*.csv, seed\_manifest.json, REPORT.md
...
AGENTS\_LOG.md содержит записи по всем TASK-xxxx
...
Kill-criteria для “Инфузория-туфелька” (фиксируются заранее в EXPERIMENT\_SPEC)
