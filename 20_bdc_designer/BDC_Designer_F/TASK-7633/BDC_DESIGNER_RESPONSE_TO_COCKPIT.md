...
## Что новое уточнение теперь подтверждает

Ваше дополнительное описание полезно тем, что снимает часть архитектурной неопределённости.

Теперь уже явно видно canonical runtime chain:
- browser UI;
- WebSocket transport;
- `ws/handler.py`;
- `session_manager.py`;
...
- `claude` subprocess;
- `stdout stream-json`;
- relay обратно в WebSocket;
- rendering через `Zustand` + React.

Это важно, потому что BDC теперь видит не только список компонентов, но и фактический execution contract:
- `Create` ведёт к реальному `session_create` событию;
- backend реально создаёт `UUID` session и отдельный `asyncio.Task`;
- `CLISession` реально работает как subprocess wrapper;
- `stdin` закрывается намеренно, то есть single-turn behavior для `--print` здесь не случайность, а часть текущего runtime design;
- `tool_use` / `tool_result` / `stream_text` уже имеют явный mapping contract;
- cockpit действительно работает с реальными файлами на диске, а не с simulated sandbox UI.

Для BDC это усиливает один вывод:
- система уже достаточно конкретна для architecture qualification;
- но bottleneck по-прежнему лежит в execution truth, а не в недостатке новых abstraction layers.
...
- особенно в:
  ...
  - stdout-event-to-WS mapping integrity,
  - reconnection semantics vs actual session continuity,
  - subprocess exit / completion truth.
...
- в какой момент закрывается `stdin` и считается ли это canonical single-turn contract;
- что именно считается завершением сессии:
  - `result`
  - process exit
  - `session_ended`
  - все три вместе.
...
- raw stdout relay vs normalized event relay
- single-client WS consumption vs multi-client broadcast consumption
...
- session_created acknowledgement latency
- first visible token latency
- follow-up success rate
- permission round-trip success rate
- session completion rate
- session recovery after reload
- WS reconnect recovery rate
- stdout JSON parse integrity
- tool event pairing integrity
...
- disconnect / dropped-event count
- mismatched event-pair count
...
Новое уточнение это фактически подтверждает:
- follow-up сейчас не просто «пока неудобен»;
- он структурно завязан на новый process boundary.

Для BDC это значит:
- проблема не в phrasing UX,
- а в том, что session continuity и interaction continuity сейчас разведены.
...
### 5. Stream relay correctness теперь становится отдельным audit surface
После вашего уточнения уже видно, что одна из реальных архитектурных поверхностей здесь:
- правильность переноса `stdout stream-json` в WS protocol и далее в UI store.

Это значит, что в реальном аудите мы будем смотреть отдельно:
- теряются ли события,
- ломается ли порядок,
- корректно ли спариваются `tool_use` и `tool_result`,
- не создаёт ли reconnect иллюзию continuity без реального восстановления состояния.
