...
def main() -> None:
    ...
    parser.add_argument(
        "--out_root",
        default="results/r3_control_resistant_sequence_memory",
    )
    parser.add_argument("--seed", type=int, default=20260319)
    parser.add_argument("--size", type=int, default=128)
    parser.add_argument("--trace_width", type=int, default=7)
    args = parser.parse_args()

    out_root = Path(args.out_root)
    out_root.mkdir(parents=True, exist_ok=True)

    dataset = build_control_resistant_sequence_memory_dataset(
        seed=args.seed,
        size=args.size,
        seq_len=10,
        alphabet_size=5,
    ...
    manifest = {
        "environment_id": "controlled_sequence_memory_control_resistant",
        "task_id": "TASK-7613",
        "seed": args.seed,
        "size": args.size,
