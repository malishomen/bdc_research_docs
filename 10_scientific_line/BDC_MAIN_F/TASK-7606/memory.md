# Project Memory

## Repository
- Path: `D:\projects\Bio_Digital_Core\Bio_digital_core`
- Main working branch: `test`
- Research mirror repo: `D:\projects\Bio_Digital_Core\bdc_research_docs`

## Current Scientific State
- `R1` selection-physics reboot is complete.
- Canonical scientific verdict:
  - `PASS_TO_R2`
- Canonical gate artifact:
  - `results/selection_physics_reboot_r1_full/aggregates/r1_gate_decision.json`
- Full-run summary:
  - `180 / 180` runs passed across 6 required regimes.

## Current Analytical State
- Full `R1` packet was run through `BDC Designer`.
- Packet was accepted as `supported`.
- `BDC Designer` winner prior for the full packet:
  - `no_penalty_diagnostic`
- This is a secondary analytical result, not a replacement for the canonical scientific gate.

## Interpretation Rule
- Canonical scientific truth:
  - `R1 = PASS_TO_R2`
- `BDC Designer` truth:
  - secondary narrowing layer only
- Do not let `BDC Designer` replace the scientific gate.

## Current Next Phase
- Active preparation target:
  - `R2 - Controlled Task Environment`
- Canonical R2 docs:
  - `docs/experiments/EXP-0802_R2_CONTROLLED_TASK_ENVIRONMENT.md`
  - `docs/experiments/BDC_R2_CONTROLLED_TASK_ENVIRONMENT_PREEXPERIMENT_GATE.md`
  - `docs/experiments/EXP-0803_R2_CONTROLLED_TASK_ENVIRONMENT_IMPLEMENTATION_PACKAGE.md`

## Current Task Chain
- `TASK-7602` - R2 foundation package
- `TASK-7603` - candidate environment matrix
- `TASK-7604` - baseline and measurement harness
- `TASK-7605` - R2 gate audit and environment approval packet

## BDC Designer Rule
- For scientific reboot work, `BDC Designer` must be used first as pre-experiment narrowing/evidence discipline.
- Direct experiment execution comes after that gate, not before.

## Research Mirrors
- Temp mirror:
  - `D:\projects\Bio_Digital_Core\Temp\research\BDC_MAIN_F`
- Designer mirror:
  - `D:\projects\Bio_Digital_Core\Temp\research\BDC_Designer_F`
- Research docs repo:
  - `D:\projects\Bio_Digital_Core\bdc_research_docs`

## Current Stop Point`r`nIf a new session starts, the correct next action is:`r`n1. use `controlled_sequence_memory` as the conservative R2 implementation prior,`r`n2. build measured candidate-specific evidence on top of the new deterministic artifact,`r`n3. rerun the bounded R2 gate on measured rather than inferred support,`r`n4. do not widen to organism/cell claims.`r`n
