...
## Current R5 Matrix State
- `R5` transfer target matrix is now defined.
- Current matrix prior:
  - `symbolic_micro_corpus_cloze_transfer`
- Reason:
  - it is the only adjacent candidate with executable task infrastructure already present in the repository.
- Deferred candidate:
...
7. treat `symbolic_micro_corpus_cloze_transfer` as the current matrix prior, not yet as the approved target,
8. execute `TASK-7625` next,
9. do not open organism or cell claims,
10. do not open micro-assembly until it gets its own explicit bounded evidence gate.
